export default () => {
    ...
    function _processFiles(name, files) {
        ...
        for (i = 0, ii = queueFiles.length; i < ii; i++) {
            (function (file) {
                _queueFile.call(_this, name, file);
            })(queueFiles[i]);
        }
    ...
    Upload.prototype.new = function (name, options) {
        ...
        input.onchange = function () {
            _onFileInputChange.call(_this, name);
        }
    ...
    Upload.prototype.files = function (name) {
        ...
        return (this.watch.uploaders[name] || {}).files || {
            all: [],
            queued: [],
            progress: [],
            uploaded: [],
